...
void ApplicationFunctionSet::CMD_MotorControlSpeed_xxx0(void)
{
  ...
  if (Application_ConquerorCarxxx0.Functional_Mode == CMD_MotorControl_Speed)
  {
    ...
    else
    {
      ...
      int left_speed = CMD_is_MotorSpeed_L;
      int right_speed = CMD_is_MotorSpeed_R;
      if (CMD_is_MotorSpeed_L <0 && CMD_is_MotorSpeed_R <0){
        
        left_speed=left_speed*-1;
        right_speed=right_speed*-1;
        
        Serial.print("{l_b"+String(uint8_t(left_speed))+",r_b"+String(uint8_t(right_speed))+"}");
        AppMotor.DeviceDriverSet_Motor_control(/*direction_A*/ direction_back, /*speed_A*/ uint8_t(left_speed),
                                             /*direction_B*/ direction_back, /*speed_B*/ uint8_t(right_speed),
                                             /*controlED*/ control_enable); //Motor control
      }else if (CMD_is_MotorSpeed_L >0 && CMD_is_MotorSpeed_R <0){
        right_speed=right_speed*-1;  
        Serial.print("{l_f"+String(uint8_t(left_speed))+",r_b"+String(uint8_t(right_speed))+"}");
        AppMotor.DeviceDriverSet_Motor_control(/*direction_A*/ direction_just, /*speed_A*/ uint8_t(left_speed),
                                             /*direction_B*/ direction_back, /*speed_B*/ uint8_t(right_speed),
                                             /*controlED*/ control_enable); //Motor control
      
     }else if (CMD_is_MotorSpeed_L >0 && CMD_is_MotorSpeed_R >0){
        Serial.print("{l_f"+String(uint8_t(left_speed))+",r_f"+String(uint8_t(right_speed))+"}");
        AppMotor.DeviceDriverSet_Motor_control(/*direction_A*/ direction_just, /*speed_A*/ uint8_t(left_speed),
                                             /*direction_B*/ direction_just, /*speed_B*/ uint8_t(right_speed),
                                             /*controlED*/ control_enable); //Motor control
     }else if (CMD_is_MotorSpeed_L <0 && CMD_is_MotorSpeed_R >0){
        left_speed=left_speed*-1;
        Serial.print("{l_b"+String(uint8_t(left_speed))+",r_f"+String(uint8_t(right_speed))+"}");
        AppMotor.DeviceDriverSet_Motor_control(/*direction_A*/ direction_back, /*speed_A*/ uint8_t(left_speed),
                                             /*direction_B*/ direction_just, /*speed_B*/ uint8_t(right_speed),
                                             /*controlED*/ control_enable); //Motor control
     }
